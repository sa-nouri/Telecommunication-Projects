% HTFE (High-Throughput Frequency Estimation)
% This script performs high-throughput frequency estimation on a received signal.

function [freq] = htfe(signal, fs)
    % HTFE Performs high-throughput frequency estimation.
    %
    % Args:
    ...
    freq = (peak_idx - length(signal)) * fs / length(signal);
end

classdef HTFE < matlab.System
...
methods (Access = protected)
  ...
  function [validPacket, cfgNonHT, rxNonHTData, chanEst, noiseVarEst, data,cfo_est,channel_est] = stepImpl(obj, x)    
    ...
    chanEst     = complex(0);
    noiseVarEst = 0;
    cfo_est = obj.pCoarseCFOEst ;
    channel_est = obj.pChanEst ;
